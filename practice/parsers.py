...
def get_text(node, selector, default = ""):
    """
        HTML 태그 안에서 '텍스트'만 안전하게 추출하는 함수

        Args: 
            node: 찾을 범위가 되는 HTML
            selector: 찾고 싶은 요소의 CSS 선택자
            default: 요소를 못 찾았을 때 사용한 기본값
    """
...
def parse_rate(text, default=None):
    """
        '+3.5%', '-1.08%' 처럼 부호(+, -)와 소수점이 포함된 텍스트에서 숫자 부분만 실수(float) 형태로 추출하는 함수
    """
    if not text:
        return default
...
def parse_stocks(html):
    """
        ...
        동작 순서
        1. 텍스트 형태의 HTML을 BeatifulSoup을 이용하여 DOM 구조로 변환
        2. 목록에 해당하는 컨테이너(tr.stock-row) 단위로 먼저 추출
        3. 해당 컨터이너에서 이름, 가격, 링크 등을 안전하게 추출하여 리스트로 담아서 반환
    """
        ...
        results.append(
            {
                ...
                "market": get_text(row, "td.col-market"),
