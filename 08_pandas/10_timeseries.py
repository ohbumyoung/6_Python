"""
    시계열 데이터 : 시간의 흐름에 따라 분포된 데이터 (시간, 수치/데이터)
"""
...
dow = df['date'].dt.day_of_week.value_counts().sort_index()
for k, v in dow.items():
    print(f" {'월화수목금토일'[k]}요일 {v:,}건")

# resampls : 시간 단위로 바꿔서 다시 묶어줌
...
monthly_vol =one['close'].resample('ME').sum()
...
# => 기간별 시가, 고가, 저가, 종가 네개열을 한번에 만들수 있음
