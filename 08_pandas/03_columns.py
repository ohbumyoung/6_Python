...
print(f"'temp'열 삭제 후 day shape : {day.shape}")

print('-'*60)

# 열 이름 변경 df.rename(columns={기존열이름 : 변경할열 이름, ...})
...
# ValueError: could not conver string to floot : 'N/A'

converted = pd.to_numeric(sample2, errors='coerce')
# errors = 'coerce' 변환 중 오류 발생 시 해당 값을 결측으로 처라
