"""
    ...
    numpy(넘파이) 기반의 데이터 구조를 제공하고, SQL처럼 다양한 조작 및 분석 기능을 제공하는 라이브러리
"""
import pandas as pd
...
"""
    * Series (시리즈) : 라벨링된 1차원 배열 구조
      - 인덱스(라벨)를 따로 지정하지 않으면  정수로 자동 생성됨
"""
datas =  [10,20,30,40]
...
print(f"values : {s.values} ({type(s.values).__name__})")   # 실제 저장된 데이터
print('-'*60)
...
df = pd.read_csv(RAW_PATH, 
                 encoding=ENCODING, 
                 parse_dates=["date"], # 'date'열을 datetime 으로 처리
                 na_values=["N/A","-"], # 결측으로 취급할 문자들
                 thousands=","          # '1,000,000' 이 형태를 숫자로 처리
                 )
print(df.head())
print(f"close dtype : {df['close'].dtype}")
print(f"date dtype : {df['date'].dtype}")
...
# date 열에는 2026-06.. 랑 202606... 이랑 2026.06 형태들록 섞여있음
...
"""
    ...
    parse_dates : 특정 열을 datetime으로 변환
    na_values : 결측으로 취급할 문자열 지정
    thoisands : 천 단위 구분자 제거 (천단위 구분자가 있는 데이터를 숫자로 변환)
"""
