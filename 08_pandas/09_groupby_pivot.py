...
#  s.shift() -> 원본과 길이가 같은 Serise를 반환 i번째 값 = s[i-1], 멘 첫 행은 NaN
...
print(f" -> {len(filterd)}행 {filterd['code'].nunique()}종목")
...
# 피벗 -형식 바꾸기
"""
    pivot_table : 한 열은 행으로, 다른 열을 열로 펼처서 집계
...
# df.melt(id_vars=유지할_열, var_name=열이름을 _담을_열, value_name=값을_담을_열)
# => 열마다 흩여져 있는 값을 하나의 열에 모으기 위함
long = wide.reset_index().melt(id_vars="sector",var_name="quarter", value_name="close")
print("== 긴 형식 == ")
print(long.head(6).round())
# DB 저장, 시작화할 때 활용

# 피봇 -> 사람이 보는 보고서, 엑셀, 데이터 확인 활용
