...
class Account:
    ...
    # 정적 메소드 : 객체, 클래스와 무관한 기능을 담당하는 메소드(유틸리티)
    @staticmethod
    def is_valid_amount(amount):
        return amount >= Account.MIN_DEPOSIT
    
acc = Account("오범영", 10000)
print(f"deposit --> {acc.deposit(3000)}")   # 인스턴스 메소드 호출

# 클래스 메소드 호출
acc2 = Account.from_dict({"owner":"오범영", "balance":10000})
...
# form_dict 활용
response = [
    {"owner": "오범영", "balance": 20000},
    {"owner": "사범영"},
    {"owner": "삼범영", "balance": 55000}
]
...
# 데이터 구조가 변겅되거나 메소드를 수정하는 경우 새로 정의해서 대응 가능함