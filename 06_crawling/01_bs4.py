"""
    ...
    2. bs4의 find, select 를 활용해서 특정 텍스를 추출
"""
import requests
...
# bs4는 문자열을 DOM 트리처럼 다룰 수 있게 만들어줌
...
# select : CSS 선택자를 사용하여 해당 요소들을 반환. 없는 경우 []
# select_one : CSS 선택자를 사용하여 해당 요소 1개 반환. 없는 경우 None
...
# f-string 에서 !r을 사용하면 repr() 호출되어 숨겨진 공백(\n, \t 등)까지 그대로 출력
...
# 첫번째 행의 전체 데이터를 추출
for sel in ["td.col-code", "td.col-name a", "td.col-sector", "td.col-price", "td.col-change", "td.col-volume", "td.col-markte span"]:
