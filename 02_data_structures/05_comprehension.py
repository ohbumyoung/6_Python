...
# 리스트 컴프리헨션 => [표현식 for 변수 in 반복되는 대상]
...
# 딕셔너리 컴프리헨션 => {키_표현식 : 밸류_표현식 for 변수 in 반복대상 if조건}
menus = ["갈비탕", "돈가스", "제육볶음", "햄부기 햄북스딱스 햄부기온앤온"]

# 키 -> 메뉴명, 밸류 -> 메뉴명의 길이
menus_dict = {m : len(m) for m in menus}
print(f"menus_dict : {menus_dict}")

menus_dict = {m : len(m) for m in menus if len(m) == 16}
print(f"menus_dict : {menus_dict}")

# 셋(set) 컴프리헨션 -> {표현식 for 변수 in 반복대상 if 조건}
message = 'No pain, No gain'

# 문자열에서 고유한 문자만 추출 (중복된 문자는 제와)
