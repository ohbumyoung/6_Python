...
# -화면(GUI 창)을 열지 않고, 메모리 내에서만 차트 표시(랜더링)
...
# Figure, Axes
# -Figure (도회지) -> Axes (그래프 하나)

# * subplots(행, 열, figsize=(가로,세로))
# 행, 열 생략 시 1 * 1

fig, ax = plt.subplots(figsize=(12,4))
# figsize => 12*4. dpi(100) --> 1200 * 400픽셀.
...
# plt.show() => 화면에 띄워서 바로 차트를 확인
# savefig()와 show() 같이 사용하는 경우, savefig() 호출 후 show()호출 해야 함
...
# axhline(y) : y 위치에 가로 기준선을 표시, lw은 선 굵기
...
#   axes -> (2,2) 배열로 반환
# * sharex : x축을 모두 공유하는 설정
fig, axes = plt.subplots(2, 2, figsize=(13,6), sharex=True)

# axes.flat : 2차원 배열을 1차원으로 펼치는 속성
