...
# 파이썬의 리스트는 데이터를 구분하지 않고 담을 수 있는 점이 편리할 수 있지만 다른 리스트와 연산을 하고자 할 때는 문제가 될 수 있음
"""
...
# 넘파이 배열 (nbarray)은 데이터 타입이 하나로 고정된 구조. -> 연산이 쉬움!
...
# 넘파이 배열은 자바의 배열과 비슷하지만, 연산 수행 시 반복문을 사용하지 않고 배열 단위의 연산이 가능
...
list-bytes = list_container + (int_obj * n)

arr_bytes = arr_10m.nbytes

print(f"데이터 {n:,} 개 기준")
print(f" - 리스트 : {list_bytes / 1024 / 1024:6.2f} MB")
print(f" 배열 : {arr_bytes / 1024 / 10246:}")