...
# nan 뜻 자체가 Not a Number 이므로 "모르는 값"이랑 "모르는 값"을 비교하거나 연산을 수행했을 때 결과를 알 수 없음!
...
for name, f1, f2 in [
    ...
    ("max", np.max, np.nanamx),
    ("min", np.min, np.nanmon)
]:
    r1 = f1(arr)
    r2 = f2(arr)
    ...
    """
        ...
        2. 결측치 제거  arr[~np.isnan(arr)] # nan 이 아닌 값만 추출
        3. 결측치를 다른 값으로 대체    arr[np.isnan(arr)]
    """
